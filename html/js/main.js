...
Promise.all([
  ...
  .then(function() {
      ...
      el.addEventListener(
        ...
        function() {
          stop().then(function() {
            ...
            el.innerHTML =
              "Wheel With Fake Scroll " + (active ? "[ON]" : "[OFF]");
    ...
    _.each(document.querySelectorAll("#main-top-navbar,#navbtns-wrp"), function(
      elm
    ) {
  ...
  .then(function() {
    if (!config.__did_quick_setup) {
      window.location = "intro"; // goto quick-setup.html page
  ...
  .then(function() {
    ...
    if (!tree_element || !tree_wrp_element)
      return Promise.reject(
        new Error("Cannot find #tree and/or #tree-wrp element")
      );
  ...
  .then(function() {
    ...
    if (
      config.message_bar_have_custom_height &&
      config.message_bar_height > 0 &&
      config.message_bar_height <= 100
    ) {
      var html = document.querySelector("html");
      var topbar_offset = 50 + (html.classList.contains("ios") ? 30 : 0);
      var msgbar_wrp = document.querySelector("#message-bar-wrp");
      if (msgbar_wrp) {
        msgbar_wrp.style.height =
          "calc(" + config.message_bar_height + "vh - " + topbar_offset + "px)";
      }
    }
    ...
    if (config.message_bar_font_size_percentage > 0) {
      var msgbar_wrp = document.querySelector("#message-bar-wrp");
      if (msgbar_wrp) {
        msgbar_wrp.style.fontSize =
          config.message_bar_font_size_percentage + "%";
      }
    }
  })
  .then(function() {
...
document.addEventListener(
  ...
  function(evt) {
    ...
    if (
      ...
      !html.classList.contains("has-fake-scroll")
    ) {
      // prevent scrolling
      evt.preventDefault();
    }
...
var _modes = ["auto", "switch", "wheel"],
  _all_delegates = {
    tree_go_in: _tree_go_in,
    tree_go_out: _tree_go_out,
    tree_go_previous: _tree_go_previous,
    tree_go_next: _tree_go_next,
  },
  _debug_keys = {
    "80": {
      func: function() {
...
window.addEventListener(
  "x-keycommand",
  function(ev) {
    if (!state || state._keyhit_off) return;
    if (!NativeAccessApi.keyCodeByInput.hasOwnProperty(ev.detail.input)) return;
    var code = NativeAccessApi.keyCodeByInput[ev.detail.input];
    ev.charCode = code;
    // look for delegate calls
    var delegate = _debug_keys[code];
    if (delegate) {
      if (delegate.preventDefault === undefined || delegate.preventDefault)
        ev.preventDefault();
      var ret = delegate.func(ev);
      if (ret && ret.catch) ret.catch(handle_error);
    }
  },
  false
);
...
function start(_state) {
  if (state && state._start_promise) return state._start_promise;
  ...
  return (state._start_promise = _start_prepare().then(function() {
    if (tree.nodes.length == 0) throw new Error("Tree has zero length");
    if (
      "Click" in config._keyhit_delegates ||
    ...
    if (config.can_edit) {
      document
        .querySelector("#edit-mode-btn")
        .addEventListener("click", _on_edit_mode, false);
      document
        .querySelector("#edit-mode-save-btn")
        .addEventListener("click", _on_edit_save, false);
      document
        .querySelector("#edit-mode-cancel-btn")
        .addEventListener("click", _on_edit_cancel, false);
    }
    _edit_mode_toggle(!!_state.edit_mode, false);
    return _before_changeposition().then(function() {
      if (state.positions[state.positions.length - 1].index != -1) _scan_move();
      else _update_active_positions();
  ...
  function auto_next() {
    if (_state._stopped) return; // stop the loop
    _state.auto_next_dead = false;
    _state._active_timeout = setTimeout(function() {
      ...
      ? config.auto_next_first_run_delay
      : null) ||
      config.auto_next_delay ||
      500);
    function run() {
      if (_state._stopped || _state._paused) return; // stop the loop
    ...
    function delayrun(delay) {
      ...
      function on_new_move() {
        ...
        if (idx != -1) _state._stop_callbacks.splice(idx, 1);
        run();
      }
...
function _start_prepare() {
  return install_tree(tree_element, tree_data, config, state).then(function(
    _tree
  ) {
    tree = _tree;
...
function stop() {
  ...
  if (state._stop_promise) return state._stop_promise;
  return (state._stop_promise = _stop_prepare().then(function() {
    if (
      "Click" in config._keyhit_delegates ||
      "RightClick" in config._keyhit_delegates
    ) {
    ...
    var callback;
    while ((callback = state._stop_callbacks.shift()) != null) callback();
    state._stopped = true;
...
function _on_xkeycommand(evt) {
  if (!state || state._keyhit_off || state._paused) return;
  var curtime = new Date().getTime();
  if (
    config.ignore_second_hits_time > 0 &&
    state._last_keydown_time &&
  ...
  if (
    evt.detail &&
    NativeAccessApi.keyCodeByInput.hasOwnProperty(evt.detail.input)
  ) {
...
function _on_keydown(down_ev) {
  ...
  if (!config.ignore_key_release_time) {
    ...
    */
    ...
    if (delegate) {
      if (delegate.preventDefault) down_ev.preventDefault();
    }
    if (state._next_keyup)
      window.removeEventListener("keyup", state._next_keyup, false);
...
function _on_scroll(evt) {
  if (!state || state._wheel_off || state._paused) return;
  ...
  // lock-the scroll
  var scrollX =
      window.scrollX > 110 || window.scrollX < 10 ? 60 : window.scrollX,
...
function _on_wheel_subrout(deltaX, deltaY) {
  ...
  if (Math.abs(state._wheel_delta[1]) > y_threshold) {
    var n =
      Math[state._wheel_delta[1] < 0 ? "ceil" : "floor"](
        state._wheel_delta[1] / y_threshold
      ) * (config.wheel_y_reverse ? -1 : 1);
    _tree_go_n_steps(n).catch(handle_error);
    state._wheel_delta = [0, 0];
  }
}
...
function _new_move_start(moveobj) {
  return unboundPromise().then(function(defer) {
    var promise = defer[0],
      resolve = defer[1],
      reject = defer[2];
    var running_move;
    _.each(moveobj.steps, function(astep) {
      promise = promise.then(function() {
        if (running_move == state._running_move) return astep(moveobj);
    ...
    running_move = state._running_move = promise.then(onend, function(err) {
      onend();
      throw err;
    });
...
function _update_active_positions(avoidloop) {
  if (state.on_next_update_active_positions) {
    _.each(state.on_next_update_active_positions, function(f) {
      f();
    });
    delete state.on_next_update_active_positions;
  }
  var dom_elements = _.map(state.positions, function(pos) {
    return pos.tree.dom_element;
  });
  ...
  if (state.positions.length > 0) {
    ...
    if (curpos.tree.nodes[curpos.index]) {
      ...
      if (node && node.content_element) {
        node.content_element.classList.add(
          "highlight" || config.highlight_class
        );
...
function _tree_set_contentsize(percentage) {
  ...
  _.each(elms, function(elm) {
    ...
    // re-set xscale
    var xscale = xscale_from_percentage_floor(
      percentage,
      tree_contentsize_xstep
    );
...
function _move_sub_speak(type, override_msg) {
  ...
  switch (type) {
    case "cue":
      opts =
        (config.mode == "auto" && is_first_run()
          ? config.auditory_cue_first_run_voice_options
          : null) || config.auditory_cue_voice_options;
  ...
  var tmp = _.filter(opts.locale_voices, function(a) {
    return a.locale == curlocale;
  });
  if (tmp.length == 0) {
    tmp = _.filter(opts.locale_voices, function(a) {
      return a.locale.split("-")[0] == curlocale.split("-")[0];
    });
  }
  ...
  if (audio) {
    return speaku.play_audio(audio, opts).catch(function(err) {
      console.error(err);
...
function _scan_move(node, opts) {
  ...
  moveobj.node.dom_element.dispatchEvent(
    new CustomEvent("x-new-move", {
      detail: {
        node: node,
      },
    })
...
function _do_notify_move(node, notifynode, opts) {
  ...
  moveobj.steps.push(
    _move_sub_speak.bind(notifynode, "main", opts.main_override_msg)
  );
  if (node) {
  ...
  if (node) {
    moveobj.steps.push(
      _move_sub_speak.bind(node, "cue", opts.cue_override_msg)
    );
  }
...
function _do_select(node, opts) {
  ...
  moveobj.steps.push(function() {
    ...
    return _meta_true_check(node.meta["no-main"], false)
      ? Promise.resolve()
      : _move_sub_speak.call(node, "main", opts.override_msg);
  });
  speaku.stop_speaking();
  state.can_move = false;
  function un_can_move() {
...
var _tree_go_out_override_functions = [_on_override_go_in_or_out];
...
function _tree_go_out() {
  if (!state.can_move) return Promise.resolve();
  // special case
  var res;
  for (var i = 0, len = _tree_go_out_override_functions.length; i < len; i++) {
    if ((res = _tree_go_out_override_functions[i]()) != null) return res;
  }
...
function _in_check_spell_delchar(atree) {
  if (_meta_true_check(atree.meta["spell-delchar"])) {
    ...
    if (tmp && tmp[1]._concat_letters) {
      ...
      if (!concat_letters) concat_letters = tmp[1]._concat_letters = [];
      ...
      var msg,
        idx = concat_letters.lastIndexOf(" ");
      if (idx != -1) {
        msg =
          concat_letters.slice(0, idx).join("") +
          " " +
...
function _in_check_spell_default(atree) {
  ...
  if (
    _meta_true_check(atree.meta["spell-finish"]) &&
  ...
  if (tmp && _meta_true_check(tmp[0])) {
    ...
    if (atree.meta["spell-word"]) {
      ...
      if (last_word_idx == -1) {
        ...
        concat_letters.splice(
          last_word_idx,
          concat_letters.length - last_word_idx,
          " "
        );
      }
    ...
    return _before_changeposition().then(function() {
      var tmp2 =
        _get_node_attr_inherits_full(state.positions, "stay-in-branch") || tmp;
...
function _in_override_change_tree(atree) {
  ...
  if (another_tree_name) {
    return get_trees_info(default_trees_info_fn).then(function(trees_info) {
      another_tree_name = another_tree_name.toLowerCase();
      var tree_info = _.find(trees_info.list, function(a) {
        return (a.name + "").toLowerCase() == another_tree_name;
      });
      if (tree_info) {
  ...
  if (another_tree === current_tree) {
    ...
    return prepare_tree(another_tree)
      .then(function(info) {
        ...
        return get_file_data(info.tree_fn).then(function(data) {
          tree_data = data;
...
function _reset_resume_at_next_action(atree) {
  ...
  function finish() {
    ...
    return (state._stopped
      ...
      if (popup && popup.classList.contains("visible")) {
        ...
        setTimeout(function() {
          if (popup_mtext) popup_mtext.textContent = "";
          popup.classList.add("hide");
        }, 500); // wait for hide transition
      }
      // update tree dyn, before start again
      return _tree_update_subdyn(tree).then(resume); // start over
    });
  }
  function clear() {
    if (atree.content_element)
      atree.content_element.classList.remove(
  ...
  function tmp_onscroll(evt) {
    ...
    var scrollX =
        window.scrollX > 110 || window.scrollX < 10 ? 60 : window.scrollX,
      scrollY =
        window.scrollY > 110 || window.scrollY < 10 ? 60 : window.scrollY;
    window.scrollTo(scrollX, scrollY);
    window._last_scroll_x = scrollX;
    window._last_scroll_y = scrollY;
  }
  function tmp_onwheel_subrout(deltaX, deltaY) {
    wheeldelta = [wheeldelta[0] + deltaX, wheeldelta[1] + deltaY];
    var x_threshold = config.wheel_x_threshold || 30;
    var y_threshold = config.wheel_y_threshold || 30;
    if (
      Math.abs(wheeldelta[0]) > x_threshold ||
  ...
  return (state._stopped ? _napi_add_key_command() : Promise.resolve()).then(
    function() {
      tmp = document.querySelector("#navbtns");
      if (tmp && config._onscreen_navigation) {
...
function _in_check_stay_in_branch(atree) {
  var stayinbranch = _get_node_attr_inherits_full(
    state.positions,
    "stay-in-branch"
  );
  if (stayinbranch) {
  ...
  if (anode.meta["webhook"]) {
    return new Promise(function(resolve, reject) {
      var url = anode.meta["webhook"],
        ...
        skip_validating_response = _meta_true_check(
          anode.meta["webhook-skip-validating-response"]
        ),
      ...
      window
        ...
        .then(function(resp) {
          return skip_validating_response
            ? Promise.resolve()
            : resp.json().catch(function(err) {
        ...
        .then(function(respdata) {
          if (
            !skip_validating_response &&
            (respdata.status + "").toLowerCase() != "success"
          ) {
            var err = new Error(
              "webhook request was not successful: " + JSON.stringify(respdata)
            );
...
function _tree_go_in() {
  ...
  if (atree.is_leaf) {
    ...
    if (state.edit_mode) return Promise.resolve();

    // explicit finish check
    // special case
    var res;
    for (
      var i = 0, len = _tree_select_override_functions.length;
      i < len;
      i++
    ) {
      if ((res = _tree_select_override_functions[i](atree)) != null) return res;
    }
    // finish it
    // on auto mode stop iteration and on any key restart
    return _leaf_select_utterance(atree);
  } else {
    if (atree.nodes.length == 0) return Promise.resolve(); // has no leaf, nothing to do
    return _before_changeposition().then(function() {
      state.positions.push({
        tree: atree,
        index: 0,
      ...
      _on_update_select_path();
      var delay =
        state.mode == "auto" ? config.auto_next_atfirst_delay || 0 : 0;
...
function _tree_move(node) {
  ...
  return _before_changeposition().then(function() {
    ...
    while (tmp != null) {
      ...
      if (idx == -1) throw new Error("Corrupt tree!");
      positions.unshift({
        tree: tmp,
        index: idx,
      });
...
function _tree_go_previous() {
  ...
  if (position.index < 0) {
    ...
    */
    if (position.tree.nodes.length == 0) return Promise.resolve();
...
function _tree_go_n_steps(n) {
  ...
  if (position.index >= position.tree.nodes.length) {
    ...
    position.index =
      Math.ceil(Math.abs(position.index) / position.tree.nodes.length) *
        position.tree.nodes.length +
      position.index;
  }
  return _scan_move();
...
function eval_config(config) {
  ...
  */
  ...
  // add styles
  var styles = Array.isArray(config.style)
      ? config.style
      : config.style
      ? [config.style]
      : [],
  ...
  for (var i = 0, len = styles.length; i < len; ++i) {
    var el = newEl("link"),
      href = styles[i];
    for (var key in style_vars) href = href.replace(key, style_vars[key]);
    el.setAttribute("rel", "stylesheet");
    el.setAttribute("href", href);
    document.body.appendChild(el);
    config._styles.push(el);
  }
  if (!("can_edit" in config)) config.can_edit = true;
  return config;
}
...
function _get_current_spell_txt() {
  ...
  if (state && !state._stopped) {
    ...
    var tmp = _.filter(state.positions, function(a) {
      return !!a._concat_letters;
    });
    if (tmp.length > 0) {
...
function _get_prediction_spell_words(words_file, txt) {
  return get_words(words_file).then(function(wdata) {
    ...
    if (!txt) {
      // empty, simple solution would empty output
      return { words: [] };
    }
...
function _word_prediction_dynamic_nodes(anode) {
  ...
  return _get_prediction_spell_words(words_file, txt).then(function(subwdata) {
...
function _letter_prediction_dynamic_nodes(anode) {
  ...
  var alphabet =
      anode.meta["alphabet"] || config.alphabet || "abcdefghijklmnopqrstuvwxyz",
    ...
    if (!subwdata.alphabet_sorted) {
      subwdata.alphabet_sorted = _.map(_.range(alphabet.length), function(i) {
        ...
        return [
          ...
          _.reduce(
            _.map(
              _.filter(subwdata.words, function(w) {
                return w.v[txt.length] == a;
              }),
              function(a) {
                return a.w;
              }
        ...
        // sort desc order by weight, letter
        if (a[2] == b[2]) {
          if (a[1] < b[1]) {
            return -1;
          }
...
function _start_auto_insert_back(tree, content_template) {
  _.each(tree.nodes, function(anode) {
    if (!anode.is_leaf) {
      ...
      anode._back_node = tree_insert_node(
        anode,
        insert_pos,
        {
          _more_meta: {
...
function install_tree(tree_element, tree_data, config, state) {
  ...
  if (!state.edit_mode) {
    if (config.helper_back_option) {
      ...
      if (tmp) content_template = _.template(tmp.innerHTML);
      _start_auto_insert_back(
        tree,
        content_template,
        config.helper_back_option
      );
    }
  ...
  return (state.edit_mode ? Promise.resolve() : _tree_dynupdate(tree)).then(
    function() {
      // rest of parse_tree
      var content_template,
        tmp = document.querySelector("#tree-node-template");
...
function _tree_dynupdate(atree) {
  return tree_traverse_nodes_async(atree, function(node, i, nodes) {
    ...
    if (dyn_name) {
      ...
      if (module) {
        return module.render(node).then(function(res) {
...
function _tree_update_subdyn(atree, options) {
  ...
  undyn(atree);
  return (options.disable_dyn
    ? Promise.resolve()
    : _tree_dynupdate(atree)
  ).then(function() {
    var content_template,
      tmp = document.querySelector("#tree-node-template");
    if (tmp) content_template = _.template(tmp.innerHTML);
...
function _clone_tree(tree) {
  var new_tree = {};
  for (var k in tree) if (tree.hasOwnProperty(k)) new_tree[k] = tree[k];
  if (tree.nodes) {
    ...
    return {
      nodes: _.filter(
        _.map(trees_info.list, function(item) {
          if (item.tree_fn == current_tree) return null;
          return {
            text: item.name,
            meta: {
