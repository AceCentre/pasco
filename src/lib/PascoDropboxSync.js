import { BackgroundTask, PascoState, getRuntimeEnv } from '../helpers/common'
...
export default class PascoDropboxSync {
  constructor (core, tokenhandler) {
    this._core = core
    ...
    task.setPromise(new Promise(async (resolve, reject) => {
      try {
        ...
        for (let { src } of write_actions) {
          ...
          await this._fmanager.saveFileData(new URL(src, target_dir_url).href, body)
        }
        ...
        datastate = await PascoDataState.loadFromFile(new URL('pasco-state.json', target_dir_url).href, this._fmanager)
        resolve(datastate)
      } catch (err) {
        reject(err)
      }
    ...
    task.setPromise(new Promise(async (resolve, reject) => {
      try {
        ...
        for (let { src, src_url } of write_actions) {
          let body = await this._fmanager.loadFileData(src_url, { responseType: 'blob' })
