...
export default class PascoTree {
  ...
  async initAndWriteToFile (tree_file, tree_data) {
    this._tree_file_info = await this._getTreeFileInfo(tree_file)
    this._tree_data = tree_data
    await this._fmanager.saveFileData(this._tree_file_info.tree_fn, tree_data)
  }
  async initAndReadFromFile (tree_file) {
    this._tree_file_info = await this._getTreeFileInfo(tree_file)
